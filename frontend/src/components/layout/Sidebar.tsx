import { Home, PlusCircle, BarChart3, Wallet, Layers } from 'lucide-react';
...
export function Sidebar() {
  ...
  return (
    <aside className="flex w-64 shrink-0 flex-col gap-5 border-r border-border bg-sidebar p-4">
      ...
      {/* Static placeholder until a shop-settings feature exists */}
      <div className="mt-auto flex items-center gap-2.5 rounded-xl border border-border-soft bg-brand-50 p-3">
        ...
        <div className="min-w-0">
          <div className="truncate text-sm font-semibold text-ink-900">{t('shopName')}</div>
          <div className="text-xs text-ink-600">CSC ID · 6743 21XX</div>
        </div>
      </div>
    </aside>
  );
